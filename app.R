...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      style = "background-color: #FEF4E8;", width = 4,
    ...
    mainPanel(
      navbarPage(
        ...
        tabPanel(
          "Epidemic size against vaccine coverage",
...
server <- function(input, output) {
  ...
  output$vacc_type_ui <- renderUI({
    vacc_type_selectors()
  })
  output$vacc_perc_ui <- renderUI({
    vacc_perc_selectors()
  })


  c1 <- reactive(get_params(
    country = "Philippines", #  input$country,
  ...
  output$countryDemoPlot <- renderPlot({
    c1()$population_size %>%
      as_tibble_row() %>%
      pivot_longer(cols = 1:16, names_to = "ageGroup", values_to = "population") %>%
      mutate(age_ind = 1:16) %>%
